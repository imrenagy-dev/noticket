...
export function IssueModal({
    ...
    const makeEmpty = () => ({
        ...
        sprint_id:
            issue?.sprint_id?.toString() ?? defaultSprintId?.toString() ?? '',
        story_points: issue?.story_points?.toString() ?? '',
        board_order: issue?.board_order?.toString() ?? 0,
    });

    const [form, setForm] = useState(makeEmpty);
    const [checklist, setChecklist] = useState<ChecklistItem[]>(
        (issue as any)?.checklist ?? [],
    );
    ...
    function submit(e: FormEvent) {
        ...
        const payload = {
            ...
            board_order: form.board_order ? Number(form.board_order) : 0,
        };
        setProcessing(true);
            ...
            <DialogContent>
                ...
                <form onSubmit={submit} className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                        ...
                        <div className="space-y-1.5">
                            ...
                            <Input
                                ...
                                onChange={(e) =>
                                    set('story_points', e.target.value)
                                }
                                placeholder="—"
                            />
                        </div>
                    ...
                    <div className="space-y-1.5">
                        <Label>Sprint</Label>
                        <Select
                            value={form.sprint_id || 'none'}
                            onValueChange={(v) =>
                                set('sprint_id', v === 'none' ? '' : v)
                            }
                        >
                            <SelectTrigger>
                    ...
                    <div className="space-y-1.5">
                        ...
                        <textarea
                            id="issue-desc"
                            className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:ring-2 focus:ring-ring focus:outline-none"
                            rows={3}
                                ...
                                {checklist.map((item) => (
                                    ...
                                    >
                                        ...
                                        {editingId === item.id ? (
                                            ...
                                            <span
                                                className={`flex-1 rounded px-1 py-0.5 text-sm ${item.done ? 'text-muted-foreground line-through' : ''}`}
                                            >
                                                {item.text}
                                            </span>
                                        )}
